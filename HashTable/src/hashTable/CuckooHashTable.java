     ...
     /*
        for (int i = 1; i <= 20; i++) {
			...
			for (int j = 0; j < NUMS; j++) {
				H.insert(HashTest.getSaltString());
			}
			System.out.println(i + "-->" + (System.currentTimeMillis() - startTime) / NUMS);

		}
      */
		 
		CuckooHashTable<String> H = new CuckooHashTable<>(new StringHashFamily(3), 2000);
		//insert 2000000 random strings
		...
		for (int i = 1; i <= 20; i++) {
			NUMS = Math.pow(2, i);
			//count the time of each search
			long startTime = System.currentTimeMillis();  
			for (int k = 0; k < NUMS; k++) {
				H.contains(HashTest.getSaltString());
			}
			System.out.println(i + "-->" + (System.currentTimeMillis() - startTime) / NUMS);
		...
		/*
		 * for( int i = 0; i < NUMS; i++) H.contains( ""+i );
		 * 
		 * for( int i = 0; i < NUMS; i++) H.remove( ""+i );
		 */
